...
channelSchema.pre("save", async function (next) {
  if (this.isNew) {
    ...
    if (existingChannel) {
      throw new Error(`Channel for course with id ${courseId} already exists.`);
    }
...
channelSchema.pre(/^delete/ /*deleteOne OR remove*/, async function (next) {
  ...
  try {
    ...
    console.error(`Error occurred while updating course with id ${courseId}.`);
...
//-------------------------------fixing the channel and messages deletion middleware----------------------------------
...
//     } catch (error) {
//         console.error(
//             `Error occurred while updating course with id ${courseId}: ${error.message}`
//         );
...
channelSchema.pre(/^delete/ /*deleteMany*/, async function (next) {
  ...
  try {
    ...
    if (!messages) {
      throw new Error(`Messages for channel with id ${channelId} not found.`);
    }
...
// channelSchema.pre("deleteMany", async function (next) {
// 	const channelId = this._id;
